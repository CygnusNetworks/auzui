...
| `GRAYLOG_DEFAULT_STREAMS` | *(empty)* | CSV of stream IDs to restrict what auzui offers/searches; empty means all streams the token(s) can see. Enforced as an allowlist: a caller-supplied `stream_ids` is intersected with it, and a request naming only streams outside it is rejected (403) rather than silently searching the allowed set or falling back to unrestricted. Also used as the default `filter` on searches that don't specify `stream_ids` explicitly. |
